...
const AgentCard: React.FC<AgentCardProps> = ({ config, content, isLoading, isPending, isConfigMode, isLocked = false, onConfigChange }) => {
    ...
    <div className={`relative flex flex-col rounded-xl border backdrop-blur-sm transition-all duration-300 ${styleClass} ${isLoading ? 'animate-pulse' : 'hover:shadow-lg hover:shadow-blue-900/10'} h-full overflow-hidden`}>
      ...
      <div className="flex items-center gap-2 p-3 border-b border-white/5 shrink-0 bg-black/10">
        ...
        <div className="flex-1 min-w-0">
          ...
          {config.description && (
            <p className="text-[10px] text-slate-400 opacity-70 truncate leading-tight mt-1">{config.description}</p>
          )}
        </div>
        {!isConfigMode && (
      ...
      {isConfigMode && (
        <div className="p-3 border-b border-white/5 bg-black/20 space-y-3 shrink-0">
          ...
          <div>
            ...
            <div className="w-full bg-slate-900/50 border border-white/5 rounded px-2 py-1.5 cursor-not-allowed">
              <div className="text-xs text-slate-400 italic">
                {MODEL_OPTIONS.find(m => m.name === config.modelName)?.label || config.modelName}
              </div>
              {MODEL_OPTIONS.find(m => m.name === config.modelName)?.description && (
      ...
      <div className={`flex-1 p-3 md:p-4 overflow-y-auto custom-scrollbar min-h-[120px] relative`}>
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full space-y-4">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-slate-500">
                {isGeminiAgent ? '🌐 正在联网搜索实时信息' : '🧠 正在深度推理分析'}
              </p>
            </div>
            {/* 进度条 */}
            <div className="w-full max-w-[200px] h-1 bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-blue-500 to-cyan-400 animate-[loading_2s_ease-in-out_infinite]"></div>
            </div>
          </div>
        ) : displayedContent ? (
          <div className="flex flex-col h-full">
            <div className="prose prose-invert prose-sm 
              prose-headings:text-inherit prose-headings:font-bold prose-headings:text-xs prose-headings:uppercase prose-headings:tracking-wider prose-headings:my-5 prose-headings:mb-3
              prose-p:text-xs prose-p:leading-loose prose-p:my-4 prose-p:mb-3
              prose-ul:my-4 prose-ul:space-y-2.5
              prose-li:text-xs prose-li:my-2 prose-li:leading-loose
              prose-strong:text-blue-300 prose-strong:font-semibold
              prose-table:text-xs prose-table:border-collapse prose-table:my-4
              prose-thead:border-b prose-thead:border-slate-600
              prose-th:px-3 prose-th:py-2 prose-th:text-left prose-th:font-semibold prose-th:text-blue-300
          ...
          <div className="text-[11px] text-slate-400 opacity-70 italic h-full flex flex-col justify-end pb-2 leading-relaxed">
